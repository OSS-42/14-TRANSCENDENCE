...
export class ChatService {
  ...
  async isUserMemberOfRoom(userId: number, roomId: number): Promise<boolean> {
    // C'est quoi la differnece avec isAlreadyMember ?
  ...
  async validatePassword(password: string, roomName: string) {
    ...
    const pwMatches = await argon2.verify(room.hash, password);
    return pwMatches;
  }
  ...
  async isUserModeratorOfChatRoom(
    userId: number,
    chatRoomId: number
  ): Promise<boolean> {
    const user = await this.prisma.utilisateur.findFirst({
  ...
  async getBlockedUserIds(userId: number): Promise<number[]> {
    ...
    return userWithBlockedUsers.blockedUsers.map(
