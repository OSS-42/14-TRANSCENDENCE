...
interface GameContextProps {
  ...
  shouldsetRespawnPowerup: boolean;
  setShouldSetRespawnPowerup: React.Dispatch<React.SetStateAction<boolean>>;
}
...
export const GameProvider: FunctionComponent = ({ children }) => {
  const [powerupPosition, setPowerupPosition] = useState<{ x: Number, y: Number, z: number } | null>(null);
  const [shouldSetRespawnPowerup, setShouldSetRespawnPowerup] = useState(false);
  const [respawnPowerup, setRespawnPowerup] = useState<(() => void) | null>(null);
  ...
  useEffect(() => {
    if (shouldSetRespawnPowerup) {
      setRespawnPowerup(respawnPowerup);
      setShouldSetRespawnPowerup(false);
    }
  }, [shouldSetRespawnPowerup]);
  ...
  return (
    <GameContext.Provider value={{
      ...
      shouldSetRespawnPowerup, setShouldSetRespawnPowerup
       }}>
      {children}
